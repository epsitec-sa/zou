...
git config --global alias.sclean "!git clean -xdf -e packages && git submodule foreach --recursive 'git clean -xdf'"
...
git config --global alias.smaster "!git checkout master && git pull && git submodule foreach --recursive 'git checkout master && git pull'"
...
git config --global alias.vlog "!git tag -l --sort=-v:refname | grep ^v[0-9]"
